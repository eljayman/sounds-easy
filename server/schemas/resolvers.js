    ...
    Mutation: {
        addUser: async (parent, { username, email, password }) => {
          ...
          addSoundToBoard: async (parent, {soundId}, context) => {
            if (context.user) {
                return User.findOneAndUpdate({_id: context.user._id}, {$addToSet: {
                    soundboard: {soundId}}
                })
          } throw new AuthenticationError("You need to be logged in to add to a soundboard!")
        },
          removeSoundFromBoard:async (parent, {soundId}, context) => {
            if (context.user) {
                return User.findOneAndUpdate({_id: context.user._id}, {$pull: {
                    soundboard: {soundId}}
                })
